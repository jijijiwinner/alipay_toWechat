...
function rc4Base64(data, sid) {
	...
	let encrypted = CryptoJSMin.RC4.encrypt(
		srcs,
		CryptoJSMin.enc.Utf8.parse(sid),
	);
...
function md5Base64(data) {
	let encrypted = CryptoJSMin.MD5(decodeURIComponent(data)).toString(
		CryptoJSMin.enc.Base64,
	);
	return encrypted;
}
...
function getDecryptProtocol(protocol) {
	...
	try {
		...
		protocol = JSON.parse(
			CryptoJSMin.RC4.decrypt(
				protocol,
				CryptoJSMin.enc.Utf8.parse(sid),
			).toString(CryptoJSMin.enc.Utf8),
		);
	}
	catch (e) {}
	return protocol;
